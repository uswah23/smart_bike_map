...
const uthmCoordinates = ol.proj.fromLonLat([103.0902, 1.8576]);
const uthmMarker = new ol.Feature({
  geometry: new ol.geom.Point(uthmCoordinates),
  name: 'UTHM'
});
uthmMarker.setStyle(new ol.style.Style({
...
vectorSource.addFeature(uthmMarker);
...
navigator.geolocation.getCurrentPosition(pos => {
  ...
  const userPoint = ol.proj.fromLonLat(userCoords);

  const userMarker = new ol.Feature({
    geometry: new ol.geom.Point(userPoint),
    name: 'You'
  });
...
stopButton.addEventListener("click", function () {
  ...
  ws.send("STOP_BUZZER");
  fetch("https://api.telegram.org/bot8116140986:AAHNgjhbXCgcawcRrzGJOVNOPSrbMoFtG7o/sendMessage", {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ chat_id: "1032611418", text: "🛑 Buzzer manually stopped by user." })
  });
});

// WebSocket connection
const ws = new WebSocket('ws:// 192.168.112.98/gps');
...
ws.onmessage = function (event) {
  ...
  if (!bikeMarker) {
    ...
    bikeMarker.getStyle().getImage().setRotation(heading * Math.PI / 180);
    map.getView().setCenter(coords);
  }
  ...
  // Draw route from user to bike (if available)
  if (userCoords) {
    fetch('https://api.openrouteservice.org/v2/directions/foot-walking/geojson', {
      ...
      body: JSON.stringify({ coordinates: [userCoords, [data.lon, data.lat]] })
    })
    .then(res => res.json())
    .then(geojson => {
      ...
      routeFeature.setStyle(new ol.style.Style({
        stroke: new ol.style.Stroke({ color: '#00cc00', width: 3 })
      }));
